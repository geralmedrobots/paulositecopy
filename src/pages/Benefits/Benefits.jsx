...
function Benefits() {
  ...
  return <>
    ...
    <Section id="workflows" alt className="solutions-section"><div className="section-intro"><div><p className="eyebrow">Application domains</p><h2>Automation in service of hospital workflows.</h2></div><p className="section-sub">These are application domains for hospital logistics systems. They are not presented as a list of currently deployed services.</p></div><div className="workflow-domains">{workflowDomains.map((domain, index) => <div key={domain}><span>0{index + 1}</span>{domain}</div>)}</div><Button to="/projects/pharmarobot">See PharmaRobot</Button></Section>
  </>;
}
